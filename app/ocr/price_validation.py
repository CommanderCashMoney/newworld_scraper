...
class PriceValidator:
    def __init__(self, price_list: List) -> None:
        ...
        self.bad_indexes = []
        self.last_validated_index = 0

    def validate_price(self, idx: int) -> bool:
        price = self.price_list[idx].get("price")
        if price is None or not price.strip(".").isnumeric():
            return False  # could check if prev/next price are the same price
        if "." not in price:
            return False  # could check if inserting a . makes it sensible
        return True

    def validate_quantity(self, idx: int) -> bool:
        qty = self.price_list[idx].get("avail", "")
        return qty.isnumeric()

    def validate_name(self, idx: int) -> bool:
        name = self.price_list[idx].get("name")
        if name is None:
            return False
        # try look it up
        ...
    ...
    def validate_next_batch(self) -> None:
        last_idx_to_validate = len(self.price_list) - 1
        for idx in range(self.last_validated_index, last_idx_to_validate):
            idx_to_validate = idx + self.last_validated_index
            if (
                not self.validate_name(idx_to_validate)
                or not self.validate_price(idx_to_validate)
                or not self.validate_quantity(idx_to_validate)
            ):
                self.bad_indexes.append(idx_to_validate)
            self.last_validated_index = idx_to_validate
